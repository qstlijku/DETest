...
bool TextureShaderClass::Initialize(ID3D11Device* device)
{
	...
	error = wcscpy_s(vsFilename, 128, L"../../code/EngineTest/texture.vs");
	if(error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"../../code/EngineTest/texture.ps");
	if(error != 0)
	{
		return false;
	}
	...
	result = InitializeShader(device, vsFilename, psFilename);
	if(!result)
	{
		return false;
	}
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
